...
int write_solution(solution * __solution, const char * filename)
{
    ...
    fprintf(file, "%u%c## Somme des retards%s", solution_get_tardiness(*__solution), OUTPUT_SEPARATOR, OUTPUT_NEW_LINE);
    /* Ligne 2 : Temps CPU (millisecondes) */
    fprintf(file, "%u%c## Temps CPU (millisecondes)%s", solution_get_cpu_time(*__solution), OUTPUT_SEPARATOR, OUTPUT_NEW_LINE);
    ...
    for(id_group = 1; id_group <= nb_groups; ++id_group)
    {
        ...
        {
            fprintf(file, "%u%c", job_get_id(jobs[index]), OUTPUT_SEPARATOR);
        }
    }
    fprintf(file, "## Ordonnancement%s", OUTPUT_NEW_LINE);
    /* Ligne 4 : Nombre de tournées == nombre de groupes */
    fprintf(file, "%u%c## Nombre de tournées%s", nb_groups, OUTPUT_SEPARATOR, OUTPUT_NEW_LINE);
    /* Ligne 5 : Nombre de sites par tournées */
    for(id_group = 1; id_group <= nb_groups; ++id_group)
    {
        fprintf(file, "%u%c", solution_get_nb_elements_in_group(*__solution, id_group), OUTPUT_SEPARATOR);
    }
    ...
    for(id_group = 1; id_group <= nb_groups; ++id_group)
    {
        ...
        for(index = 0; index < nb_elements_in_group; ++index)
        {
            fprintf(file, "%u%c", site_get_id(sites[index]), OUTPUT_SEPARATOR);
        }
        fprintf(file, "## Tournée %u%s", id_group, OUTPUT_NEW_LINE);
    }
