...
# Le troisième paramètre ne doit pas être un fichier (erreur #4)
			if [ ! -f $3 ]
			then
# S'il y a un quatrième paramètre, il doit s'agir d'un entier positif (erreur #5)
				if [ $# -eq 4 ]
				then
					if [ ! $4 -gt 0 ]
# Gestion de l'erreur #5
					then
						echo "***  Erreur : \"$4\" n'est pas un entier strictement positif ! ***"
						exit
					fi
				fi
# Si le troisième paramètre représente un dossier à créer, alors il faut le créer
				...
				then
					...
					mkdir $3
# Vérification de la création du dossier représenté par le troisième paramètre (erreur #6)
					if [ -d $3 ]
					then
